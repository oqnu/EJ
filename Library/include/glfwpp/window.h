...
namespace glfw
{
	...
	protected:
		using PosEventType = Event<Window&, int, int>;
		using SizeEventType = Event<Window&, int, int>;
		using CloseEventType = Event<Window&>;
		using RefreshEventType = Event<Window&>;
		using FocusEventType = Event<Window&, bool>;
		using IconifyEventType = Event<Window&, bool>;
		using MaximizeEventType = Event<Window&, bool>;
		using FramebufferSizeEventType = Event<Window&, int, int>;
		using ContentScaleEventType = Event<Window&, float, float>;
		PosEventType posEvent;
		SizeEventType sizeEvent;
		CloseEventType closeEvent;
		RefreshEventType refreshEvent;
		FocusEventType focusEvent;
		IconifyEventType iconifyEvent;
#if GLFW_VERSION_MAJOR >= 3 && GLFW_VERSION_MINOR >= 3
		MaximizeEventType maximizeEvent;
#endif
		FramebufferSizeEventType framebufferSizeEvent;
#if GLFW_VERSION_MAJOR >= 3 && GLFW_VERSION_MINOR >= 3
		ContentScaleEventType contentScaleEvent;
#endif
	...
	protected:
		using KeyEventType = Event<Window&, KeyCode, int, KeyState, ModifierKeyBit>;
		using CharEventType = Event<Window&, unsigned int>;
		using MouseEventType = Event<Window&, MouseButton, MouseButtonState, ModifierKeyBit>;
		using CursorPosEventType = Event<Window&, double, double>;
		using CursorEnterEventType = Event<Window&, bool>;
		using ScrollEventType = Event<Window&, double, double>;
		using DropEventType = Event<Window&, std::vector<const char*>>;
		KeyEventType keyEvent;
		CharEventType charEvent;
		MouseEventType mouseButtonEvent;
		CursorPosEventType cursorPosEvent;
		CursorEnterEventType cursorEnterEvent;
